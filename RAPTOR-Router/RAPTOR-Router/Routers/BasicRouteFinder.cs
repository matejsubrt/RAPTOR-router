...
namespace RAPTOR_Router.Routers
{
    ...
    public class BasicRouteFinder : IRouteFinder
    {
        ...
        internal BasicRouteFinder(Settings settings, RAPTORModel raptorModel)
        {
        ...
        private void ImproveByTransfers()
        {
            ...
            {
                return searchModel.GetEarliestArrivalInRound(transfer.To, round) > searchModel.GetEarliestArrivalInRound(transfer.From, round).AddSeconds(transfer.GetTransferTime(settings.WalkingPace));
            }
            void ImproveArrivalByTransfer(Transfer transfer)
            {
                searchModel.SetEarliestArrivalInRound(transfer.To, round, searchModel.GetEarliestArrivalInRound(transfer.From, round).AddSeconds(transfer.GetTransferTime(settings.WalkingPace)));
