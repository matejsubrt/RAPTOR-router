...
namespace RAPTOR_Router.RAPTORStructures
{
	...
	public class SearchResult
	{
		...
		(
			...
			{
				if(usedTrips.Count > 0)
				{
					departureTime = usedTrips[0].StopTimes[usedTrips[0].Route.RouteStops.IndexOf(getOnStops[usedTrips[0]])].DepartureTime.AddSeconds(-(int)(usedTransfers[0].GetTransferTime(settings.WalkingPace)*settings.GetMovingTransferLengthMultiplier()));
				}
				else
				{
					departureTime = TimeOnly.FromDateTime(startingTime);
				}
			}
			else
			{
				departureTime = usedTrips[0].StopTimes[usedTrips[0].Route.RouteStops.IndexOf(getOnStops[usedTrips[0]])].DepartureTime;
			}

			if (startsWithTransfer)
			{
				Transfer firstTransfer = usedTransfers[0];
				Transfers.Add(new UsedTransfer { segmentIndex = 0, srcStop = firstTransfer.From.Name, destStop = firstTransfer.To.Name, distance = firstTransfer.Distance, time = firstTransfer.GetTransferTime(settingsUsed.WalkingPace) });
				transferIndex++;
				segmentIndex++;
			}
			...
			while (tripIndex < usedTrips.Count && transferIndex < usedTransfers.Count)
			{
				...
				segmentIndex++;
				tripIndex++;
				Transfers.Add(new UsedTransfer { segmentIndex = segmentIndex, srcStop = currTransfer.From.Name, destStop = currTransfer.To.Name, distance = currTransfer.Distance, time = currTransfer.GetTransferTime(settingsUsed.WalkingPace) });
				segmentIndex++;
				transferIndex++;
			}
			if (endsWithTransfer)
			{
				Transfer lastTransfer = usedTransfers[usedTransfers.Count - 1];

                Trip lastTrip = usedTrips[usedTrips.Count - 1];
				StopTime lastTripGetOffStopTime = lastTrip.StopTimes[lastTrip.Route.RouteStops.IndexOf(getOffStops[lastTrip])];
				TimeOnly lastTripGetOffStopTimeOnly = lastTripGetOffStopTime.ArrivalTime;

				int secondsToAdd = lastTransfer.GetTransferTime(settings.WalkingPace);

                SetArrivalDateTime(startingTime, lastTripGetOffStopTimeOnly.AddSeconds(secondsToAdd));
			}
			...
			if (!endsWithTransfer)
			{
				...
				//ArrivalTime = lastTrip.StopTimes[lastTrip.Route.RouteStops.IndexOf(getOffStops[lastTrip])].ArrivalTime;
				SetArrivalDateTime(startingTime, lastTrip.StopTimes[lastTrip.Route.RouteStops.IndexOf(getOffStops[lastTrip])].ArrivalTime);
			}
		}

		void SetArrivalDateTime(DateTime departureDateTime, TimeOnly arrivalTime)
		{
			TimeOnly departureTime = TimeOnly.FromDateTime(departureDateTime);
			DateOnly departureDate = DateOnly.FromDateTime(departureDateTime);
			if(departureTime <= arrivalTime)
			{
				ArrivalDateTime = DateTimeExtensions.FromDateAndTime(departureDate, arrivalTime);
			}
			else
			{
				ArrivalDateTime = DateTimeExtensions.FromDateAndTime(departureDate.AddDays(1), arrivalTime);
			}
		}
		...
		public override string ToString()
		{
			...
			{
				return "Transfer from " + Transfers[0].srcStop + " to " + Transfers[0].destStop + ", length: " + Transfers[0].time + "s + reserve " + (settingsUsed.GetMovingTransferLengthMultiplier() - 1.0) * Transfers[0].time + "s = " + Transfers[0].distance + "m";
			}
			...
			if (Trips[0].segmentIndex == 0)
			{
				departureTime = Trips[0].getOnTime;
			}
			else
			{
				departureTime = Trips[0].getOnTime.AddSeconds(-Transfers[0].time);
			}

			if (Trips[Trips.Count -1].segmentIndex > Transfers[Transfers.Count - 1].segmentIndex)
			{
				arrivalTime = Trips[Trips.Count - 1].getOffTime;
			}
			else
			{
				arrivalTime = Trips[Trips.Count - 1].getOffTime.AddSeconds(Transfers[Transfers.Count - 1].time);
			}
			...
			while(segmentIndex <= Math.Max(Trips[Trips.Count-1].segmentIndex, Transfers[Transfers.Count - 1].segmentIndex))
			{
				...
				{
					sb.AppendLine("Transfer from " + Transfers[transferIndex].srcStop + " to " + Transfers[transferIndex].destStop + ", length: " + Transfers[transferIndex].time + "s + reserve " + (settingsUsed.GetMovingTransferLengthMultiplier() - 1.0) * Transfers[transferIndex].time + "s = " + Transfers[transferIndex].distance + "m");
					transferIndex++;
					segmentIndex++;
				}
			}
			...
			sb.AppendLine("Penalty seconds: " + Transfers.Count*settingsUsed.GetTransferPenaltySeconds());
			sb.AppendLine("Total time w penalties: " + length.Add(new TimeSpan((long)Transfers.Count * settingsUsed.GetTransferPenaltySeconds() * 10_000_000)));
			return sb.ToString();
		}
